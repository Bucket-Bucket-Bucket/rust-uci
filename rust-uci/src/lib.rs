...
use libuci_sys::{
    uci_alloc_context, uci_commit, uci_context, uci_delete, uci_element, uci_free_context, uci_get_errorstr, uci_list,
    uci_lookup_ptr, uci_option_type_UCI_TYPE_LIST, uci_option_type_UCI_TYPE_STRING, uci_ptr,
...
impl Drop for UciPtr {
    fn drop(&mut self) {
        unsafe { let _ = CString::from_raw(self.1); };
    }
}
